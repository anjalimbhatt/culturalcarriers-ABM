...
# bsub -q short -n 16 -R "rusage[mem=5G]" -M 15G -hl -o ./log/googlemodel_varygoogle_%J.out -B -N Rscript ./src/culturalcarriers_fixedinitial_googlemodel_varygoogle_202407.R
# 
# Coded Apr 2024: Behemoth Model (aka Google)
# Changed params July 2024: vary Google parameters
# 
# author: Anjali Bhatt
...
filename <- paste("data/large/", Sys.Date(), "_results_googlemodel_varygoogle.csv", sep="") # output file
...
params <- CJ(
  ...
  s1 = c(1.0), # hiring selectivity threshold
  
  # behemoth params
  # s0_google = s0, # assume base rate of random entry for behemoth is same as others
  r1_google = seq(0.1,1,0.1), # turnover alienation for behemoth
  b1_google = seq(0,1,0.1), # socialization susc for behemoth
  s1_google = seq(0.1,1,0.1) # selectivity for behemoth
