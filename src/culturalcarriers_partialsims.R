...
Partial set of simulations over parameter grid
For plotting
...
n_sims <- 3 # number of simulations per set of parameters
...
params <- CJ(
  ...
  eps_soc = c(0, 0.1), # socialization error term (standard deviation)
  ...
  eps_hire = c(0, 0.1) # hiring error term (standard deviation) 
  )
...
culture_fn <- function(par) {
  ...
  name <- paste("plots/", "soc", par$socialize, "_alien", par$alienate, "_sel", par$select, "_r", par$r1, "_s", par$s0,
                "_esoc", par$eps_soc, "_ehire", par$eps_hire, "_no", par$sim_no, ".png", sep="")
